...
const coordsToAddr = async (coords) => {
  ...
  let qu = `https://reverse.geocoder.cit.api.here.com/6.2/reversegeocode.json?&app_id=${process.env.APP_ID}&app_code=${process.env.APP_CODE}&prox=${codQu}&mode=retrieveAddresses&maxresults=1`;
  const response = await axios.get(qu);
  const data = response.data["Response"]["View"][0]["Result"][0]["Location"]["Address"]["Label"];
  return data;
}
